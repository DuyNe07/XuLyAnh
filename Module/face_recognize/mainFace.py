...
def getData(nameUser, col):
    ...
    output_dir = f'./Module/face_recognize/image/{nameUser}/'
    ...
    while True:
        ...
        if faces[1] is not None:
            ...
            file_name = f'./Module/face_recognize/image/{nameUser}/{nameUser}_{dem:03d}.bmp'
...
def Training(col):
    with col.status("Processing training", expanded=True) as status:
        ...
        metadata = load_metadata('./Module/face_recognize/image')
        ...
        for i, m in enumerate(metadata):
            ...
            joblib.dump(svc, './Module/face_recognize/model/svc.pkl')
...
def predict(type, img, col):
    svc = joblib.load('./Module/face_recognize/model/svc.pkl')
    tm = cv.TickMeter()
    mydict = []
    pathImg = './Module/face_recognize/image'
...
result_path = '.\\Module\\face_recognize\\output'
